...
public class SetupSpawnAddCommand implements SubCommandExecuter {
    @Override
    public boolean onCommand(CommandSender commandSender, Command cmd, String s, String[] args, String command) {
        ...
        if (arena != null) {
            arena.addSpawnLocation(Bukkit.getPlayer(commandSender.getName()).getLocation());
            commandSender.sendMessage("Player Spawn Position für " + arenaName + " wurde auf deine Position gesetzt");
