...
public class SetupLobbyCommand implements SubCommandExecuter {
    @Override
    public boolean onCommand(CommandSender commandSender, Command cmd, String s, String[] args, String command) {
        ...
        if (arena != null) {
            arena.setLobby(Bukkit.getPlayer(commandSender.getName()).getLocation());
            commandSender.sendMessage("Lobby Position für " + arenaName + " wurde auf deine Position gesetzt");
